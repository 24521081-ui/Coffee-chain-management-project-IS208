...
public class WarehouseTransferDAO {
    ...
    public void createTransfer(WarehouseTransferSlip slip) throws DatabaseException {
        try (Connection conn = DBConnection.getConnection()) {
            ...
            try {
                insertHeader(conn, slip);
                insertLines(conn, slip);
                conn.commit();
            } catch (SQLException e) {
                conn.rollback();
    ...
    public List<WarehouseTransferSlip> findTransfersForApproval(String branchId) throws DatabaseException {
        String sql = """
                SELECT pdc.phieu_dieu_chuyen_id, pdc.kho_nguon_id, pdc.kho_dich_id,
                       pdc.nhan_vien_id, pdc.ngay_dieu_chuyen, pdc.trang_thai,
                       kn.ten_kho || ' - ' || cnn.ten_chi_nhanh AS ten_kho_nguon,
                       kd.ten_kho || ' - ' || cnd.ten_chi_nhanh AS ten_kho_dich,
                       (SELECT COUNT(*) FROM chi_tiet_phieu_dieu_chuyen_kho ct
                         WHERE ct.phieu_dieu_chuyen_id = pdc.phieu_dieu_chuyen_id) AS so_mat_hang
                ...
                ORDER BY pdc.ngay_dieu_chuyen DESC
                """;
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    Timestamp createdAt = rs.getTimestamp("ngay_dieu_chuyen");
                    slip.setCreatedAt(createdAt == null ? null : createdAt.toLocalDateTime());
                    slip.setStatus(DB_STATUS_PENDING);
                    slip.setLines(loadLines(conn, slip.getSlipId()));
                    slips.add(slip);
                }
            }
            return slips;
    ...
    public void approveTransfer(String slipId) throws DatabaseException, ValidationException {
        try (Connection conn = DBConnection.getConnection()) {
            conn.setAutoCommit(false);
            try {
                WarehouseTransferSlip slip = lockTransfer(conn, slipId);
                ...
                updateStatus(conn, slipId, DB_STATUS_APPROVED);
                conn.commit();
            } catch (SQLException e) {
                conn.rollback();
    ...
    public void rejectTransfer(String slipId) throws DatabaseException, ValidationException {
        try (Connection conn = DBConnection.getConnection()) {
            conn.setAutoCommit(false);
            try {
                lockTransfer(conn, slipId);
                updateStatus(conn, slipId, DB_STATUS_REJECTED);
                conn.commit();
            } catch (SQLException e) {
                conn.rollback();
    ...
    private void insertHeader(Connection conn, WarehouseTransferSlip slip) throws SQLException {
        String sql = """
    ...
    private void insertLines(Connection conn, WarehouseTransferSlip slip) throws SQLException {
        String sql = """
    ...
    private List<WarehouseTransferLine> loadLines(Connection conn, String slipId) throws SQLException {
        String sql = """
    ...
    private WarehouseTransferSlip lockTransfer(Connection conn, String slipId) throws SQLException, ValidationException {
        String sql = """
                SELECT phieu_dieu_chuyen_id, kho_nguon_id, kho_dich_id, nhan_vien_id, ngay_dieu_chuyen, trang_thai
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                Timestamp createdAt = rs.getTimestamp("ngay_dieu_chuyen");
                slip.setCreatedAt(createdAt == null ? null : createdAt.toLocalDateTime());
                slip.setLines(loadLines(conn, slipId));
                return slip;
            }
        }
    }

    private void updateStatus(Connection conn, String slipId, String status) throws SQLException {
